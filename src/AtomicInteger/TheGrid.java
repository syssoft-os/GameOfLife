...
public class TheGrid {
    ...
    public TheGrid ( int horizontal_dimension, int vertical_dimension ) {
        this.horizontal_dimension = new AtomicInteger(horizontal_dimension);
        this.vertical_dimension = new AtomicInteger(vertical_dimension);
        this.grid = new AtomicBoolean[horizontal_dimension][vertical_dimension];
    }

    public void setCell ( int x, int y, boolean value ) {
        this.grid[x][y] = new AtomicBoolean(value);
    }
    ...
    public int countLivingNeighbors(int x, int y) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                int dx = x+i < 0 ? horizontal_dimension.get() - 1 : (x+i) % horizontal_dimension.get();
                int dy = y+j < 0 ? vertical_dimension.get() - 1 : (y+j) % vertical_dimension.get();
    ...
    public int getHorizontalDimension () {
        return horizontal_dimension.get();
    }

    public int getVerticalDimension () {
        return vertical_dimension.get();
    }

    public void print (PrintStream p) {
        for (int row = 0; row < vertical_dimension.get(); row++) {
            for (int col = 0; col < horizontal_dimension.get(); col++) {
                if (grid[col][row].get()) {
                    p.print("X");
                } else {
                    p.print(".");
    ...
    public TheGrid nextGeneration() {
        TheGrid next = new TheGrid(horizontal_dimension.get(), vertical_dimension.get());
        for (int col = 0; col < horizontal_dimension.get(); col++) {
            for (int row = 0; row < vertical_dimension.get(); row++) {
    ...
    private AtomicInteger horizontal_dimension;
    private AtomicInteger vertical_dimension;
